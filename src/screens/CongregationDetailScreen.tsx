...
import { CongregationDetails } from "../api/googleSheets";
import NavigationBar from "../components/NavigationBar";

type Props = NativeStackScreenProps<RootStackParamList, "CongregationDetail">;

export default function CongregationDetailScreen({ route, navigation }: Props) {
  const { congregation } = route.params;

  const openDirectionsInMaps = async () => {
    if (!congregation.address || congregation.address === "N/A") {
      Alert.alert("Address Not Available", "No address information is available for this congregation.");
      return;
    }

    try {
      // Create search query for the destination
      const destination = `${congregation.name}, ${congregation.address}`;
          ...
          >
            <View className="px-6 py-6">
              ...
              <View className="space-y-4">
                ...
                {/* Contact */}
                {congregation.contact && congregation.contact !== "N/A" && (
                  <View className="bg-white rounded-2xl p-5 border border-gray-200 shadow-sm">
                    <View className="flex-row items-center mb-3">
                ...
                {/* Email */}
                {congregation.email && congregation.email !== "N/A" && (
                  <View className="bg-white rounded-2xl p-5 border border-gray-200 shadow-sm">
                    <View className="flex-row items-center mb-3">
                ...
                {congregation.description && congregation.description !== "N/A" && (
                  <View className="bg-white rounded-2xl p-5 border border-gray-200 shadow-sm">
                    <View className="flex-row items-center mb-3">
                ...
                {congregation.address === "N/A" && congregation.contact === "N/A" && congregation.email === "N/A" && congregation.description === "N/A" && (
                  <View className="bg-gray-50 rounded-3xl p-8 items-center">
                    <Ionicons name="information-circle-outline" size={48} color="#9CA3AF" />
                    <Text className="text-gray-600 text-center mt-4">
                      No additional details available for this congregation
                    </Text>
                  </View>
                )}
              </View>
