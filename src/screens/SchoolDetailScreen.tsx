...
import { SchoolDetails } from "../api/googleSheets";
import NavigationBar from "../components/NavigationBar";

type Props = NativeStackScreenProps<RootStackParamList, "SchoolDetail">;

export default function SchoolDetailScreen({ route, navigation }: Props) {
  const { school } = route.params;

  const openDirectionsInMaps = async () => {
    if (!school.location || school.location === "N/A") {
      Alert.alert("Location Not Available", "No location information is available for this school.");
      return;
    }

    try {
      // Create search query for the destination
      const destination = `${school.name}, ${school.location}`;
          ...
          >
            <View className="px-6 py-6">
              ...
              <View className="space-y-4">
                ...
                {/* Contact */}
                {school.contact && school.contact !== "N/A" && (
                  <View className="bg-white rounded-2xl p-5 border border-gray-200 shadow-sm">
                    <View className="flex-row items-center mb-3">
                ...
                {/* Programs */}
                {school.programs && school.programs !== "N/A" && (
                  <View className="bg-white rounded-2xl p-5 border border-gray-200 shadow-sm">
                    <View className="flex-row items-center mb-3">
                ...
                {/* History */}
                {school.history && school.history !== "N/A" && (
                  <View className="bg-white rounded-2xl p-5 border border-gray-200 shadow-sm">
                    <View className="flex-row items-center mb-3">
                ...
                {school.location === "N/A" && school.contact === "N/A" && school.email === "N/A" && school.programs === "N/A" && school.history === "N/A" && (
                  <View className="bg-gray-50 rounded-3xl p-8 items-center">
                    <Ionicons name="information-circle-outline" size={48} color="#9CA3AF" />
                    <Text className="text-gray-600 text-center mt-4">
                      No additional details available for this school
                    </Text>
                  </View>
                )}
              </View>
